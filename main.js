  ...
  let gameState = {
    ...
    blockedChoices: [],
    round: 1,
    enemyUsedBlockLastRound: false // ADD THIS LINE
};
...
const elements = {
    ...
    noBuffBtn: document.getElementById('no-buff-btn'),
...
function setupEventListeners() {
    ...
    // Кнопка "Без усиления"
    elements.noBuffBtn.addEventListener('click', () => {
        // Убираем выделение с баффов
        document.querySelectorAll('.buff-btn').forEach(b => b.classList.remove('selected'));
        
        if (gameState.playerBuff === 'none') {
            gameState.playerBuff = null;
            elements.noBuffBtn.classList.remove('selected');
        } else {
            gameState.playerBuff = 'none';
            elements.noBuffBtn.classList.add('selected');
        }
        updateMakeMoveButton();
    });
...
function updateMakeMoveButton() {
    const canMove = gameState.playerChoice && (gameState.playerBuff || gameState.playerBuff === 'none');
...
function generateEnemyMove() {
  // Если игрок использовал блок в прошлом раунде, исключаем один случайный выбор
  ...
  if (gameState.playerBuff === 'block') {
      ...
      availableChoices = choices.filter(choice => choice !== blockedChoice);
      resultText += `Блок заблокировал ${choiceNames[blockedChoice]} у врага! `;
  }
  
  gameState.enemyChoice = availableChoices[Math.floor(Math.random() * availableChoices.length)];
...
function calculateResult() {
    ...
    const playerBuffText = gameState.playerBuff === 'none' ? 'Без усиления' : 
                         gameState.playerBuff ? buffs[gameState.playerBuff].name : 'Нет';
...
function nextRound() {
  ...
  // gameState.enemyUsedBlockLastRound stays as is for next round
...
function updateBlockedChoices() {
  ...
  // Применяем блокировки только если враг использовал блок в прошлом раунде
  if (gameState.enemyUsedBlockLastRound) {
      const randomChoice = choices[Math.floor(Math.random() * choices.length)];
      gameState.blockedChoices = [randomChoice];
      gameState.enemyUsedBlockLastRound = false; // сбрасываем флаг
  } else {
      gameState.blockedChoices = [];
  }
